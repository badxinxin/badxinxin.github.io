...
$.ajax({
  ...
  data: {
      key: 'CHABZ-4TB3G-5LAQV-Q77NJ-GKSGH-6OFNZ',
      output: 'jsonp',
  },
  dataType: 'jsonp',
...
function showWelcome() {
  ...
  let dist = getDistance(118.852504,32.024024, ipLoacation.result.location.lng, ipLoacation.result.location.lat); //这里换成自己的经纬度
  ...
  else if (date.getHours() >= 19 && date.getHours() < 24) timeChange = "<span>晚上好</span>，再不睡就等到起死嘛！";
  else timeChange = "这个点还不睡的是铁fw。";

  try {
      //自定义文本和需要放的位置
      document.getElementById("welcome-info").innerHTML =
          `<b><center>🎉 扣你鸡娃 🎉</center>&emsp;&emsp;欢迎来自 <span style="color:var(--theme-color)">${pos}</span> 的炫家军，${timeChange}您现在距离小炫约 <span style="color:var(--theme-color)">${dist}</span> 公里，当前的IP地址为： <span style="color:var(--theme-color)">${ip}</span>， ${posdesc}</b>`;
  } catch (err) {
...
function createtime1() {
  var grt = new Date("26/04/2023 00:00:00"); //此处修改你的建站时间或者网站上线时间
...
function createtime() {
  ...
  // 网站诞生时间
  var grt = new Date("04/26/2023 00:00:00");
